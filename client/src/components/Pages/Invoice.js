...
const Invoice = () => {
  ...
  const loadData = async () => {
    const Returnresponse = await Axios.get(
      "https://flightapp-fq75.onrender.com/invoicefares"
    );
  ...
  useEffect(() => {
    ...
    Axios.post("https://flightapp-fq75.onrender.com/UpdateFlightBooking",{
        id:initialState.sc_id,
    })
    Axios.get(`https://flightapp-fq75.onrender.com/invoice/${initialState.cl_id}`).then((resp) =>
      setData({ ...resp.data[0] })
    );
    loadData();
  }, []);
  const foo = async () => {
    await Axios.post("https://flightapp-fq75.onrender.com/invoiceconfirm", {
      id: initialState.sc_id,
      departure: user.departure,
    });
    await Axios.post("https://flightapp-fq75.onrender.com/invoiceconfirmAgain", {
      id: initialState.cl_id,
      flight_no: user.flight_no,
      fares: user.price.substr(2, 6),
    });
    Axios.delete('https://flightapp-fq75.onrender.com/removeSearch');
